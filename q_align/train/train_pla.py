...
def finalize_wandb_run(training_args, trainer, config_path=None):
    """Log best-step info and upload a compact outputs artifact to W&B."""
    ...
    artifact = wandb.Artifact(
        name=f"{run.id}-outputs",
        type="model",
        metadata={
            "run_name": str(training_args.run_name),
    ...
    for rel_name in [
        "trainer_state.json", "all_results.json", "train_results.json", "eval_results.json", "test_results.json",
        "config.json", "adapter_config.json", "adapter_model.safetensors", "non_lora_trainables.bin",
    ]:
        file_path = output_dir / rel_name
        if file_path.exists():
            artifact.add_file(str(file_path), name=rel_name)
    if config_path and os.path.exists(config_path):
        artifact.add_file(str(config_path), name=f"config/{pathlib.Path(config_path).name}")
    if best_ckpt and os.path.isdir(best_ckpt):
        artifact.add_dir(best_ckpt, name="best_checkpoint")
    run.log_artifact(artifact)
    rank0_print(f"Logged W&B artifact: {run.id}-outputs")
